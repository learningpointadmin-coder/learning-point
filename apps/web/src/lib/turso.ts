// Use the explicit HTTP subpath so the bundler resolves to lib-esm/http.js
// (the main entry's `workerd` export points at lib-esm/web.js, which Next.js's
// standalone tracer fails to copy — breaking the Cloudflare/OpenNext build).
// HTTP transport is exactly what Turso recommends for serverless/edge runtimes
// and works identically in Node dev.
